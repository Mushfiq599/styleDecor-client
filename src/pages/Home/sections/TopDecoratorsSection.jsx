    ...
    axios.get("http://localhost:5000/users/decorators")
      .then(res => {
        const realDecorators = res.data.map(d => ({
          _id: d._id,
          name: d.name,
          specialty: d.specialty || ["Decoration"],
          rating: d.rating || 4.8,
          projects: d.projects || 0,
          image: d.photo || `https://ui-avatars.com/api/?name=${encodeURIComponent(d.name)}&background=0D9488&color=fff&size=200`,
          isReal: true,
        }))
        const needed = Math.max(0, 4 - realDecorators.length)
        const fillerDummies = dummyDecorators.slice(0, needed)
        setDecorators([...realDecorators, ...fillerDummies])
      })
      .catch(() => {
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {decorators.map((decorator, i) => (
            <motion.div
              ...
              {decorator.isReal && (
                <span className="absolute top-3 right-3 px-2 py-0.5 rounded-full bg-primary text-white text-xs font-body">
                  Verified
                </span>
              )}
